...
describe( 'Metalsmith Menu Plugin', () => {
  ...
  it( 'should create a properly nested navigation structure with permalinks', ( done ) => {
    const ms = Metalsmith( fixture( 'basic' ) )
    ...
    ms.process( ( err, files ) => {
      ...
      // Test breadcrumbs on deeply nested file
  ...
  it( 'should exclude items marked with navExclude', ( done ) => {
    const ms = Metalsmith( fixture( 'nav-exclude' ) )
      .use( navigationPlugin( {
        usePermalinks: true,
        metadataKey: 'testNav'
      } ) );
  ...
  it( 'should respect navLabel and title metadata', ( done ) => {
    const ms = Metalsmith( fixture( 'nav-label' ) )
      .use( navigationPlugin( {
        usePermalinks: true,
        metadataKey: 'testNav'
      } ) );
  ...
  it( 'should handle navIndex correctly from frontmatter and options', ( done ) => {
    const ms = Metalsmith( fixture( 'nav-index' ) )
      .use( navigationPlugin( {
        usePermalinks: true,
  ...
  it( 'should generate titles from filenames when no explicit title is provided', ( done ) => {
    const ms = Metalsmith( fixture( 'no-title' ) )
      .use( navigationPlugin( {
        usePermalinks: true,
  ...
  it( 'should correctly map non-index file paths with permalinks', ( done ) => {
    const ms = Metalsmith( fixture( 'deep-paths' ) )
      .use( navigationPlugin( {
        usePermalinks: true,
        metadataKey: 'testNav'
      } ) );
  ...
  it( 'should correctly map directory paths without permalinks', ( done ) => {
    const ms = Metalsmith( fixture( 'docs' ) )
      .use( navigationPlugin( {
        usePermalinks: false,
        metadataKey: 'dirNav'
      } ) );
  ...
  it( 'should exclude files marked as draft', ( done ) => {
    const ms = Metalsmith( fixture( 'draft' ) )
      .use( navigationPlugin( {
        usePermalinks: true,
  ...
  it( 'should handle all types of exclusion patterns', ( done ) => {
    const ms = Metalsmith( fixture( 'exclusion' ) )
      .use( navigationPlugin( {
        usePermalinks: true,
  ...
  it( 'should handle non-index child paths with and without permalinks', ( done ) => {
    ...
    const msPermalink = Metalsmith( fixture( 'child-paths' ) )
      .use( navigationPlugin( {
        usePermalinks: true,
        metadataKey: 'permalinkNav'
      } ) );

    msPermalink.process( ( err ) => {
      if ( err ) {
        return done( err );
      }

      // Test without permalinks
      const msHtml = Metalsmith( fixture( 'child-paths' ) )
        .use( navigationPlugin( {
          usePermalinks: false,
          metadataKey: 'htmlNav'
        } ) );
  ...
  it( 'should add the path to the file navigation object for active state detection', ( done ) => {
    const ms = Metalsmith( fixture( 'child-paths' ) )
      .use( navigationPlugin( {
        usePermalinks: true,
  ...
  it( 'should properly nest directory children under parent HTML files in non-permalink mode', ( done ) => {
    const ms = Metalsmith( fixture( 'parent-html' ) )
      .use( navigationPlugin( {
        usePermalinks: false,
    ...
    ms.process( ( err ) => {
      ...
      // Find the blog page and products page
      ...
      // Verify paths are correct (with .html extension in non-permalink mode)
